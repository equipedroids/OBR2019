...
#define ID_BLACK 975
#define LEFT_WHITE 900
#define RIGHT_WHITE 883
#define DEFAULT_DEG 63
...
/* ARDUINOS EXTERNOS */
#define LEFT_COLOR_NANO 11
#define RIGHT_COLOR_NANO 12
...
void setup() {
  ...
  pinMode(LEFT_COLOR_NANO, INPUT);
  pinMode(RIGHT_COLOR_NANO, INPUT);
  delay(2000);
}
...
void deviateObstacles(){
  ...
  readGyroSensor();
  if(distance >= 5 && distance <= DEFAULT_DISTANCE){
    turnGyro(DEFAULT_DEG, "left", true);
    motors.stop();delay(1000);
...
/* FUNÇÃO PARA IDENTIFICAR REDUTORES */
void identifyMotorStall(){
  readGyroSensor();
  if(yaw == lastYaw && (actualTime-motorStallLastTime) >= 5000){
    motors.drive(55, FORWARD);
    delay(600);
    moveTo("backward");
    delay(200);
    motorStallLastTime = actualTime;
  }else if(yaw != lastYaw){
    Serial.println("moviemnto");
    motorStallLastTime = actualTime;
  }
  lastYaw = yaw;
}
